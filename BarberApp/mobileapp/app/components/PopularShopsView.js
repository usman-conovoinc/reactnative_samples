...
export default PopularShopsView = ({ navigation }) => {

    const [barbers, setBarbers] = useState([]);

    const getPopularBarbers = async () => {
        try {
            const response = await fetch(BASE_URL + '/api/barbers/popular?count=2', {
                method: 'GET',
                headers: {
    ...
    return (
        <View style={styles.mainView}>
            ...
            <View style={styles.titleAndSeeAllContainer}>
                <Text style={styles.title}>Popular Shops Near You</Text>
                <TouchableOpacity>
